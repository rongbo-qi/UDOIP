    ...
    def connect(self):
        try:
            ...
            with open(self.poi_trans_centers_file, 'r', encoding='utf-8') as file:
                ...
                if type(self.poi_trans_centers[0]['poi_from_center']) != int:
                    for item in self.poi_trans_centers:
                        item['poi_from_center'] = int(item['poi_from_center'])
                        item['poi_to_center'] = int(item['poi_to_center'])
                    # 更新原文件
                    with open(self.poi_trans_centers_file, 'w', encoding='utf-8') as files:
                        json.dump(self.poi_trans_centers, files, ensure_ascii=False, indent=4)
